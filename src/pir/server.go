...
func Eval() {
	...
	bytes, err := ioutil.ReadFile("./jsondata/data.json")
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, p := range persons {
		...
		ans += (fServer.EvaluatePF(byte(server_id), FssKeys_party, p.ID)) * p.AnnualIncome
	}

	f, _ = os.Create("./results/party" + os.Getenv("SERVER_ID"))
	enc := gob.NewEncoder(f)
